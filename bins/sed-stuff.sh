...
if [[ -f "$file_in" ]]; then
  ...
  do
    ...
    if [[ ${the_line:0:2} == "S:" ]]; then _str_=$(echo "${the_line:2}" | tr -d "\r"); echo "str:[$_str_]"; fi
    if [[ ${the_line:0:2} == "R:" ]]; then _rpl_=$(echo "${the_line:2}" | tr -d "\r"); echo "rpl:[$_rpl_]"; fi
    if [[ ${the_line:0:2} == "F:" ]]; then _fil_=$(echo "${the_line:2}" | tr -d "\r"); echo "fil:[$_fil_]"; fi
    if [[ "$_fil_" != "" ]]; then
      if [[ -f "$_fil_" ]]; then
